...
mkdir -p $HOME/.claude/hooks $HOME/.claude/agents $HOME/.claude/bin
...
cp $SCRIPT_DIR/bin/wt $HOME/.claude/bin/wt
chmod +x $HOME/.claude/bin/wt
...
for pattern in '.claude/worktrees/' '.claude/.env-ready' '.plans/' '.worktrees/'; do
